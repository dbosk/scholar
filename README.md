...
| Scopus | `SCOPUS_API_KEY` | Yes | [dev.elsevier.com](https://dev.elsevier.com); the key is IP-bound to the institution, so off campus set `SCOPUS_INST_TOKEN`. A 400 "Error translating query" on every query means Elsevier's search service refuses the key, not that the query is wrong |
